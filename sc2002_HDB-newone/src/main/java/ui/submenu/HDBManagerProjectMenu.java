...
public class HDBManagerProjectMenu extends ProjectMenu{
    ...
    private void createProject(List<Project> projects) {
        ...
        // Check for overlapping application periods for the same manager
        for (Project existingProject : projects) {
            if (existingProject.getManagerInCharge().getNric().equals(((HDBManager) this.user).getNric())) {
                
                Date existingStart = existingProject.getApplicationOpeningDate();
                Date existingEnd = existingProject.getApplicationClosingDate();
        
                // Overlap condition: if newStart <= existingEnd && existingStart <= newEnd
                if (!applicationClosingDate.before(existingStart) && !applicationOpeningDate.after(existingEnd)) {
                    System.out.println("You already have a project during this application period (" +
                        new SimpleDateFormat("yyyy-MM-dd").format(existingStart) + " to " +
                        new SimpleDateFormat("yyyy-MM-dd").format(existingEnd) + ").");
                    System.out.println("A manager can only manage one project per application period.");
                    return;
                }
            }
        }
        ...
        if (officerSlots > 10) {
            System.out.println("Error: Maximum number of officer slots is 10. Setting to 10.");
            officerSlots = 10;
        }
     ...
     * 
     * @param projects The list of projects to search for the project to edit
