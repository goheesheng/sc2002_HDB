...
public class HDBOfficer extends Applicant {
    ...
    public boolean registerForProject(Project project) {
        ...
        // Check if the officer has a pending application as an applicant
        if (getApplicationStatus() == ApplicationStatus.PENDING) {
            System.out.println("You cannot register as an officer while you have a pending application as an applicant.");
            return false;
        }
    ...
    public boolean applyForProject(Project project) {
        // Check if this applicant is an officer managing this project
        if (project.getOfficers().contains(this)) {
            System.out.println("You cannot apply for a project that you are managing.");
            return false;
        }
        ...
        for (Registration reg : registrations) {
            if (reg.getStatus() == RegistrationStatus.PENDING) {
                System.out.println("You cannot apply as an applicant while you have a pending officer registration.");
                return false;
            }
        }
        
        if (getAppliedProject() == null && project.isEligibleForUser(this) && project.isWithinApplicationPeriod()) {
            ...
            // Check if there are remaining flats
            int remainingFlats = project.getRemainingFlats(flatType);
            if (remainingFlats > 0) {
                // Create and add the application without decrementing flat count
            ...
            if (getAppliedProject() != null) {
                System.out.println("You have already applied for a project.");
            } else if (!project.isWithinApplicationPeriod()) {
                System.out.println("This project's application period has ended.");
            } else {
                System.out.println("You are ineligible for this project.");
            }
        }
    ...
    /**
     * Determines the eligible flat type for this officer based on marital status.
     * 
